...
//             </div>
//
//             <div className={`${!isHovering ? 'hidden' : 'flex'} w-full transition-all mt-4 md:-mt-4 items-baseline ${project.links.length === 2 ? 'justify-between' : 'justify-center'}`}>
...
function ProjectContainer({project}) {
    ...
    return (
        <div
            ...
            onClick={() => isSmallScreen && setIsHovering(isClicked => !isClicked)}
        >
            <div>
                {project.image && <img className="rounded-md transition-all" src={project.image} alt={project.title}/>}
                <h3 className="my-2 text-base font-medium transition-all text-titleColorLightTheme dark:text-gray-100 md:my-3 md:text-lg">{project.title}</h3>
            ...
            <div
                className={`${!isHovering ? 'hidden' : 'flex'} w-full transition-all mt-4 md:-mt-4 items-baseline ${project.links.length === 2 ? 'justify-between' : 'justify-center'}`}>
                {project.links.map((link, index) => (
                    <a href={link} target="_blank">
                        {index % 2 === 0
                            ? <button
