function ProjectContainer({project}) {
    return (
        <div
            ...
            <h3 className="my-2 md:my-3 text-base md:text-lg font-medium transition-all text-titleColorLightTheme dark:text-gray-100">{project.title}</h3>
            <ul>{project.technologies.map((t, i) => <li
                className="text-xs py-[1px] md:text-sm transition-all text-textColorLightTheme dark:text-textColorDarkTheme"
                key={i}>{t}</li>)}</ul>
        </div>
    );
}
